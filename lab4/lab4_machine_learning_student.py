...
movieNameWithAvgRatingsRDD = (moviesRDD
                              ...
                              .sortByKey()
                              .map(lambda (movie_id,(title,(count,avg))): (avg,title,count)))
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda ((UserID, MovieID), (pred, actual)): (math.pow(pred-actual,2))))
...
trainingAvgRating = trainingRDD.map(lambda (UserID, MovieID, rating): rating).reduce(add) / trainingRDD.count()
print 'The average rating for movies in the training set is %s' % trainingAvgRating

testForAvgRDD = testRDD.map(lambda (UserID, MovieID, ranking): (UserID, MovieID, trainingAvgRating))
...
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda (movieID,title): movieID not in [int(i[1]) for i in myRatedMovies])
...
predictedRDD = predictedRatingsRDD.map(lambda(user, movie, pred): (movie, pred))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD)
                       .map(lambda (m_id, ((pred, numRating), title)): (pred, title, numRating)))
