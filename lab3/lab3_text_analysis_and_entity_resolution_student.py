...
# TODO: Replace <FILL IN> with appropriate code
amazonRecToToken = amazonSmall.map(lambda (id,line): (id,tokenize(line)))
googleRecToToken = googleSmall.map(lambda (id,line): (id,tokenize(line)))
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda (id,tokens): len(tokens)).reduce(add)
...
def tf(tokens):
    ...
    for key,value in tf.iteritems() :
        tf[key] = float(value) / float(len(tokens))
    return tf
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.flatMapValues(lambda x:x).distinct()
    tokenCountPairTuple = uniqueTokens.map(lambda (doc_id,token):(token,1)).reduceByKey(add)

    tokenSumPairTuple = tokenCountPairTuple.map(lambda (token,count):(token,float(N)/float(count)))
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens)
    tfIdfDict = {k: idfs[k]*tfs[k] for k in tfs}
    return tfIdfDict
...
def dotprod(a, b):
    ...
    """
    prod=0
    for term in (set(a.keys()) & set(b.keys())):
        prod += a[term]*b[term] 
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda (url,a_id,sim):(' '.join([a_id,url]), sim))

trueDupsRDD = (sims
               .join(goldStandard))
trueDupsCount = trueDupsRDD.count()
avgSimDups = trueDupsRDD.map(lambda (id,(sim,gold)):sim).reduce(add) / float(trueDupsCount)

nonDupsRDD = (sims
              .subtractByKey(goldStandard))
avgSimNon = nonDupsRDD.map(lambda (id,sim):sim).reduce(add) / float(nonDupsRDD.count())
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.map(lambda (id,line): (id,tokenize(line)))
googleFullRecToToken = google.map(lambda (id,line): (id,tokenize(line)))
...
amazonWeightsRDD = amazonFullRecToToken.map(lambda (id,tokens):(id,tfidf(tokens, idfsFullBroadcast.value)))
googleWeightsRDD = googleFullRecToToken.map(lambda (id,tokens):(id,tfidf(tokens, idfsFullBroadcast.value)))
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.mapValues(norm)
amazonNormsBroadcast = sc.broadcast(amazonNorms.collect())
googleNorms = googleWeightsRDD.mapValues(norm)
...
amazonInvPairsRDD = (amazonWeightsRDD
                    .map(lambda (id,dicts):(id,dicts.keys()))
                    .flatMap(invert)
                    .cache())
